...
**Prettier handles import organization automatically on save** for the best developer experience.
...
4. Internal packages/components (using `@/` paths)
...
- **Prettier**: Handles code formatting AND import organization (on save!)
- **ESLint**: Handles unused import removal and other linting rules
- **VS Code**: Prettier runs on save to format and organize imports automatically
- **Git hooks**: Ensure both formatting and linting before commit

**Why this approach:**

- ✅ Imports organized **automatically on save** - no manual commands needed
- ✅ Consistent import organization across the entire codebase
- ✅ Works seamlessly with VS Code auto-save
- ✅ ESLint still handles unused import detection and removal
...
6. **Import organization**: Imports are organized by Prettier on save. If not working, check that Prettier extension is enabled and `format on save` is active
